...
::: {.theorem}
  **Theorem.** 
  ...
  Then we have that
  $$
    \bar{h}_n - \mathbb{E}_f[h(X)] \stackrel{d}{\to} \mathcal{N}(0, v_n)
  $$
  as $n \to \infty$.
:::
...
$$
  \frac{\sum_{j=1}^n(h(\bar{X}_j) - \mathbb{E}_f[h(X)])}{\sqrt{n}}  \stackrel{d}{\to} \mathcal{N}(0, \textrm{Var}(h(X))).
$$

Multiplying through by $\frac{1}{\sqrt{n}}$ gives the result, since $v_n = \frac{1}{n} \textrm{Var}(h(X))$.
<div style="text-align: right">
$\blacksquare$
</div>
...
Since $v_n = \frac{1}{n} \textrm{Var}(h(X))$, the standard deviation is tending to zero at a rate of $\frac{1}{\sqrt{n}}$.
